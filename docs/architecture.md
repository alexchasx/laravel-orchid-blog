...
- **Layout** `resources/views/layouts/techlog.blade.php` — шапка с навигацией и переключателем темы, `<main>`, футер, кнопка «наверх»; OG-разметка и SEO-мета (`metaTitle`/`metaDesc` или meta статьи).
- **Страницы**: `index.blade.php` (hero, masonry-сетка постов, «Темы», «О блоге», newsletter), `article.blade.php` (шапка статьи, aside TOC, prose-контент, hero-image, теги, комментарии), `contact.blade.php`, `unsubscribe.blade.php`, `dashboard.blade.php`, `errors/*` (кастомные страницы ошибок), `emails/new-article.blade.php`.
...
- **JS**: `resources/js/techlog.js` — мобильное меню, тема, scroll-reveal, to-top, валидация форм, модалки. `resources/js/app.js` — legacy/Alpine.
