    ...
    <div class="row">
        <form action="{{ route('articleUpdate') }}" method="post" role="form">
            ...
            <div class="form-group">
                ...
                <select name="category_id" size="3" class="form-control" id="category_id" required>
            ...
            <div class="form-group">
                ...
                <textarea name="description" rows="4" class="form-control" id="description" required>{{$article->description}}</textarea>
            </div>
            <div class="form-group">
                <label for="content">Полный текст</label>&nbsp;&#10033;
                <textarea name="content" rows="25" class="form-control" id="content" required>{{$article->content}}</textarea>
            </div>
            <div class="form-group">
                <input name="user_id" type="hidden" class="form-control" id="user_id" value="{{Auth::user()->id}}">
            </div>
            <div class="form-group">
                <label for="published">Показывать?</label>
            ...
            <div class="form-group">
                <label for="tag_ids">Все теги:</label>
                <select name="tag_ids[]" size="5" class="form-control" id="tag_ids" multiple>
                    @foreach ($tags as $tag)
                    <option @if (1===$tag->id)
                        selected
                        @endif
        ...
        <table>
            ...
            <tr>
                <td>{{$tag->title}}</td>
                <td>
                    <form action="{{ route('articleTagDelete', ['article'=>$article->id, 'tag'=>$tag->id]) }}" method="post">
