...
- **SEO:** единый источник мета-тегов — `layouts/techlog.blade.php` + `config/seo.php` (дефолты для title, description, OG, Twitter, Organization). Контроллеры передают `metaTitle`/`metaDesc` через `?:` (пустая строка заменяется на дефолт из config). `canonical` — self для каждой страницы, `?page=N` убирается. `robots` — `index,follow` по умолчанию; `noindex,follow` для поиска, `page>=2`, `/notpublic`, `/dashboard`, `/profile`, `/unsubscribe`, `/consent/revoke`, тестовых страниц. JSON-LD (`includes/jsonld.blade.php`) выводится **только** если `noindex` НЕ в `$metaRobots`. Sitemap — `SitemapController` (кастомный, без пакетов), кэш 1 час, включает главную, статьи, рубрики, теги, статические страницы. robots.txt блокирует `/admin`, `/dashboard`, `/profile`, `/login`, `/register`, `/test-*`, `/notpublic`, `/unsubscribe`, `/consent/revoke`, `?search=`, `?page=`. URL рубрик/тегов — slug (`{rubric:slug}`, `{tag:slug}`); legacy `/rubric/{id}` и `/tag/{id}` → 301. H1 — ровно один на страницу: на главной hero `<h1>`, на рубрике/теге — `<h1>` с названием. Хлебные крошки — `includes/breadcrumbs.blade.php` + JSON-LD BreadcrumbList. gzip — в `docker/nginx/conf.d/nginx.conf` (level 6).
