...
### Requirement: Выпадающее меню «Темы»

Кнопка `.nav-dropdown-toggle` **SHALL** переключать видимость `.nav-dropdown-menu`:

- При клике на кнопку: меню получает класс `.open`, `aria-expanded="true"`; повторный клик — закрывает
- При клике вне `.nav-dropdown`, нажатии `Escape` или клике на ссылку внутри меню — меню закрывается
- При открытии/закрытии мобильного меню (`.menu-toggle`) выпадающие списки закрываются
- `aria-expanded` **SHALL** отражать состояние меню (`true`/`false`); `aria-controls` ссылается на `#nav-topics-menu`

#### Scenario: Меню открывается по клику

- **WHEN** пользователь нажимает «Темы» в хедере
- **THEN** `.nav-dropdown-menu` получает класс `.open` и `aria-expanded="true"`

#### Scenario: Меню закрывается по клику вне

- **WHEN** пользователь кликает в любое место вне выпадающего меню
- **THEN** `.open` удаляется и `aria-expanded` становится `false`

#### Scenario: Меню закрывается по Escape

- **WHEN** меню открыто и пользователь нажимает `Escape`
- **THEN** `.open` удаляется
